...
from python_core.motifs.motifs2 import count_motifs as count_motifs2
...
from test_core.util import Colors, Loader, time_function_p
...
motifs2, _ = time_function_p(lambda: count_motifs2(hg, order))

print(f"{Colors.DIM}Running motifs3... {Colors.RESET}")
motifs3, _ = time_function_p(lambda: count_motifs3(hg, order))


rep, rep_map = generate_motifs(order)
count = 0
error_count = 0
for motif in rep:
    ...
    if (
        True
        or motifs2[motif].count != motifs3[motif].count
        or not isclose(motifs2[motif].intensity, motifs3[motif].intensity)
    ):
        error_count += 1
