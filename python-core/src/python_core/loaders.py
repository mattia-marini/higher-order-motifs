...
def load_facebook_hs(construction_method: ConstructionMethodBase = StandardConstructionMethod(), only_confirmed=True):
    ...
    if not construction_method.silent:
...
def load_random_hypergraph(N, E):
    hg = Hypergraph()
    for _ in track(range(E), description="Generating random hypergraph"):
        s = random.randint(2, 3)
        e = random.sample([i for i in range(1, N + 1)], s)
        while hg.has_edge_with_nodes(e):
            e = random.sample([i for i in range(1, N + 1)], s)
...
def load_PACS_common():
    ...
    dataset = f"{cfg.DATASET_DIR}/PACS.csv"

    # Scan lazily and read only the columns we need
    lf = pl.scan_csv(dataset, schema=dtypes, null_values=[""])

    # Build a mapping FullName -> author_id (first-seen order preserved)
    names_lf = (
        lf.select("FullName")
        .filter(pl.col("FullName").is_not_null())
        .unique(maintain_order=True)
        .with_row_index("author_id")
    )

    # Join lazily and aggregate per ArticleID. Collect only the final grouped result.
    grouped_lf = (
        lf.join(names_lf, on="FullName", how="left")
        .group_by("ArticleID", maintain_order=True)
        .agg(
            pl.col("author_id").unique().alias("authors"),  # list of unique author_ids per paper
            pl.col("PACS").first().alias("PACS"),
        )
    )
...
def load_DBLP(construction_method: ConstructionMethodBase = StandardConstructionMethod()):
    ...
    #     count = 0
    #     for row in track(reader, desciption="Loading csv file"):
    #         paper, author, y = row
    #         # y = int(y)
