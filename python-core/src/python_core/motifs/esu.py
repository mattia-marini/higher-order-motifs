...
def esu(hg: Hypergraph, order: int) -> tuple[int]:
    ...
    count = 0
    reps, rep_map = generate_motifs(order)
    counts = {motif: 0 for motif in reps}
    # rep_map = {k: 0 for k in rep_map}
    ...
    def graph_extend(sub, ext, v, n_sub):
        ...
        if len(sub) == order:
            ...
            if labeled_motif in rep_map:
                counts[rep_map[labeled_motif]] += 1
